...
import alik.itms.QosHandle.obj.AddOBJ;
...
public class ACSCorba
{
	...
	private List<DevRpcCmdOBJ> execRPC(DevRpc[] devRpcArr)
	{
		...
		AcsCorbaDAO acsCorbaDAO = new AcsCorbaDAO(Global.getPrefixName(Global.SYSTEM_NAME)+Global.SYSTEM_ACS);
...
//	}
	...
	public int chgInfo(int chgType, int infoType, String data)
	{
		...
		return new AcsCorbaDAO(Global.getPrefixName(Global.SYSTEM_NAME)+Global.SYSTEM_ACS).chgInfo(chgType, infoType, data);
	}
		...
		try
		{
			if (null != devRPCArr && devRPCArr.length > 0) 
			{
				logger.warn("[{}]调用acs开始,systemKey={},devRPCArr={},devRPCArr.size={}",
						new Object[]{devRPCArr[0].devId,Global.SYSTEM_NAME+SVR_OBJECT_NAME,devRPCArr,devRPCArr.length});
			}
